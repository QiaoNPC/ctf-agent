---
name: vuln_attack-prepare-other
description: Prepare a generic attack recipe for a hypothesis that does not fit the more specific families, writing artifacts/out/attacks/<HID>.json.
metadata:
  short-description: Prepare generic attack recipe.
---
...
## Family focus
This preparation skill is specialized for **other**. Prioritize the following reasoning:
- the evidence is concrete but the family is mixed, uncommon, or app-specific
- the issue may involve redirect logic, archive handling, path traversal, business-rule mismatch, cache poisoning, or parser confusion
- the recipe should still reduce the problem to a clean baseline, focused variant set, and observable signal
...
- mixed-family issues should not become vague; choose the most testable path first
- if two families are genuinely involved, keep one dominant family in the attack artifact and mention the overlap in notes
- stateful or sequence-heavy issues may need explicit prerequisite branches
- browser, callback, or race requirements must be explicit, not implied
- generic artifacts fail if they are less concrete than family-specific ones
...
## Example prompts
- "Prepare the other attack recipe for hypothesis <HID>."
