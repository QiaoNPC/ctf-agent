---
name: vuln_attack-prepare-mass-assignment
description: Prepare mass-assignment test variants (add sensitive fields placeholders) for a single hypothesis into artifacts/out/attacks/<HID>.json.
metadata:
  short-description: Prepare mass assignment recipe.
---
...
## Family focus
This preparation skill is specialized for **mass-assignment**. Prioritize the following reasoning:
- patch or update endpoints that merge arbitrary input into models
- nested field injection, set-by-path helpers, merge utilities, or permissive serializers
- sensitive fields like role, owner_id, org_id, status, approved, is_admin, balance, callback_url
- discrepancies between request validation and model assignment
- create vs update differences, especially when defaults and protected fields diverge
...
- frameworks may silently ignore unknown fields, so success criteria should account for both accepted and partially applied mutations
- nested JSON, form-data, and query-to-body coercion can surface different assignment behavior
- sensitive fields may only become observable in later reads, not in the immediate write response
- model hooks may overwrite values after assignment; record that possibility
- list or bulk update endpoints may accept fields that single-item endpoints reject
...
## Example prompts
- "Prepare the mass-assignment attack recipe for hypothesis <HID>."
