---
name: vuln_attack-prepare-deserialization
description: Prepare deserialization test plan templates (format constraints only, no gadget payloads) for a single hypothesis into artifacts/out/attacks/<HID>.json.
metadata:
  short-description: Prepare deserialization recipe.
---
...
## Family focus
This preparation skill is specialized for **deserialization**. Prioritize the following reasoning:
- parser or object hydration boundaries, especially where untrusted structured data becomes typed objects
- content-type, format, compression, archive, or serialization toggles
- trust boundaries between signed, encrypted, internal, and external formats
- observable signals such as parser errors, object shape changes, ignored vs accepted fields, or unexpected class/type handling
...
- the risky behavior may be format confusion, over-permissive parsing, or unsafe type materialization rather than remote code execution
- some serializers only become dangerous with exact content types or transport wrappers; capture those prerequisites
- parser behavior may differ between create and update endpoints, sync and async workers, or API and internal job ingestion
- deserialization-like bugs can overlap with mass assignment or file import issues; name the dominant family but record overlaps
- do not include gadget chains, weaponized serialized objects, or active exploitation sequences
...
## Example prompts
- "Prepare the deserialization attack recipe for hypothesis <HID>."
