...
metadata:
  short-description: Prepare authz bypass recipe.
---
...
## Family focus
This preparation skill is specialized for **authz**. Prioritize the following reasoning:
- role boundaries such as user vs admin, org member vs org admin, support vs standard user
- alternate endpoint shapes that may miss a guard
- handler or service paths where authorization may be checked in one route but not another
- object loading that happens before policy checks
- policy decisions derived from mutable request fields or stale cached actor state
...
- different routes may hit the same service method with different middleware chains
- authorization may exist in UI and controller but not service layer, or vice versa
- support or internal roles may have hidden bypass flags or impersonation paths
- partial denials such as filtered fields or redacted responses can still indicate a meaningful authz gap
- baseline should use the actor’s own object or role-allowed action; modified request should change only the scope, role assumption, or endpoint shape
...
## Example prompts
- "Prepare the authz attack recipe for hypothesis <HID>."
